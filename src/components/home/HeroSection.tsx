...
/**
 * HeroDark — Editorial / Wirecutter-grade hero, generated via Claude Design
 * (handoff TlPDoDvvH3oEWu8bRFa6fw, 2026-04-25). Adapted to the Factory codebase:
 * Next.js Link, lucide-react ArrowRight, env-var-driven props.
 */
function HeroDark({
  ...
  // Render the Claude Design "sweep" decoration by default; allows the warm
  // primary glow + cool counterpoint to read on top of the dark base.
  return (
    <section
      className="relative isolate overflow-hidden bg-[#0a0d12] text-white antialiased"
    >
      {/* Optional full-bleed backdrop photo (env-var driven) */}
      {backgroundUrl && (
        <div
      ...
      {/* Decorative: brand-color light sweep — one strong move */}
      <div
        aria-hidden="true"
      ...
      >
        <div
          className="absolute"
          style={{
            top: '-25%', right: '-15%', width: '85%', height: '120%',
            background:
              'radial-gradient(ellipse 60% 55% at 65% 35%, hsl(var(--primary) / 0.28), hsl(var(--primary) / 0.08) 45%, transparent 70%)',
            filter: 'blur(8px)',
          }}
        />
        <div
          className="absolute"
          style={{
            bottom: '-30%', left: '-10%', width: '55%', height: '70%',
            background:
              'radial-gradient(ellipse 50% 50% at 30% 70%, rgba(56,189,248,0.08), transparent 65%)',
            filter: 'blur(20px)',
          }}
        />
      </div>

      {/* Top hairline rule — editorial masthead vibe */}
      <div
        aria-hidden="true"
        className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/15 to-transparent"
      />

      <div className="relative mx-auto w-full max-w-[1280px] px-6 pb-20 pt-16 sm:px-10 sm:pb-24 sm:pt-20 lg:px-16 lg:pb-32 lg:pt-28">
        {/* Eyebrow */}
        <div className="flex items-center gap-3">
          ...
          >
            Independent {site.niche || 'product'} reviews
          </p>
        </div>

        {/* Display headline — accent word in primary */}
        <h1
          className="mt-6 max-w-[18ch] font-extrabold text-white sm:mt-8"
          style={{
        ...
        <div className="mt-9 flex flex-wrap items-center gap-3 sm:mt-10 sm:gap-4">
          ...
          <Link
            href={primaryUrl}
            className="group relative inline-flex items-center gap-2 rounded-full px-6 py-3.5 text-[15px] font-semibold tracking-[-0.005em] transition focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0d12]"
            style={{
              background: 'hsl(var(--primary))',
              color: '#06170d',
              boxShadow:
                '0 1px 0 rgba(255,255,255,0.25) inset, 0 10px 30px -10px hsl(var(--primary) / 0.55)',
            }}
          >
          ...
          <Link
            href={secondaryUrl}
            className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.03] px-6 py-3.5 text-[15px] font-semibold text-white/90 tracking-[-0.005em] backdrop-blur-sm transition hover:border-white/30 hover:bg-white/[0.07] hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
          >
            {secondaryText}
          </Link>

          {/* Trust mini-line — editorial flourish */}
          <span className="ml-1 hidden items-center gap-2 text-[12.5px] text-white/40 sm:inline-flex">
            <svg
        ...
        {/* Metric strip */}
        <dl className="mt-12 grid grid-cols-2 gap-x-6 gap-y-8 border-t border-white/10 pt-8 sm:mt-14 sm:grid-cols-4 sm:gap-x-10 sm:pt-10">
          <Metric label="Reviews" value={postCount > 0 ? `${postCount}+` : '—'} divider={false} />
          <Metric label="Categories" value={categoryCount > 0 ? String(categoryCount) : '—'} divider />
          <Metric label="Editorial" value="Independent" divider />
          <Metric label="Edition" value={String(new Date().getFullYear())} divider />
        </dl>
      </div>

      {/* Hidden in DOM but reachable via callers/screenreaders if valueProps are set */}
      {valueProps.length > 0 && (
        <div className="sr-only">{valueProps.join(' · ')}</div>
      )}
    </section>
  );
}
...
function Metric({ label, value, divider }: { label: string; value: string; divider: boolean }) {
  return (
    <div className={`flex flex-col gap-1.5 ${divider ? 'sm:border-l sm:border-white/10 sm:pl-10' : ''}`}>
      <dt className="text-[11px] font-medium uppercase tracking-[0.18em] text-white/45">
        {label}
      </dt>
      <dd
        className="font-semibold text-white leading-none tracking-[-0.02em]"
        style={{
          ...
          {imageUrl ? (
            <div className="hidden lg:flex justify-center">
              <div className="relative w-full max-w-md">
                <div
                  className="absolute -top-5 -right-5 w-full h-full rounded-2xl border-2 border-primary/15 bg-primary/5 rotate-[3deg]"
                  aria-hidden="true"
                />
                <div
                  className="absolute -bottom-5 -left-5 w-full h-full rounded-2xl border border-primary/25 bg-background -rotate-[2deg] -z-10"
                  aria-hidden="true"
                />
                ...
                <img
                  ...
                  alt={imageAlt ?? ''}
                  loading="eager"
                  className="relative rounded-2xl border border-border object-cover w-full aspect-[4/3] shadow-2xl ring-1 ring-primary/10"
                />
              </div>
            </div>
          ) : (
...
function HeroStats({
  ...
  if (categoryCount > 0) stats.push({ value: String(categoryCount), label: 'Categories' });
  if (postCount > 0) stats.push({ value: `${postCount}+`, label: 'Expert Articles' });
  stats.push({ value: 'Free', label: 'Unbiased Reviews' });
  return (
    <div className={`mt-2 inline-flex items-stretch divide-x divide-current/0 pt-8 border-t ${borderClassName}`}>
      {stats.map((s, i) => (
        <div key={s.label} className={`px-6 sm:px-8 text-center ${i > 0 ? `border-l ${borderClassName}` : ''}`}>
          <div className={`text-3xl font-bold tabular-nums tracking-tight ${valueClassName || 'text-primary'}`}>
            {s.value}
          </div>
          <div className={`text-xs sm:text-sm uppercase tracking-wider ${labelClassName}`}>
            {s.label}
          </div>
        </div>
      ))}
...
function getValuePropositions(siteType: string, niche: string | null): string[] {
  ...
  const propositionsByType: Record<string, string[]> = {
    ...
    reviews: ['Research-based reviews', 'Unbiased ratings', 'Real user feedback'],
  };
