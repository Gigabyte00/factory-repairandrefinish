...
import { Prose, Callout, LastUpdated, ProductCallout, InlineTOC, AudienceFit, Sources, Changelog, RelatedInline, BountyCTA, type ChangelogEntry } from '@/components/content';
          ...
          <article>
            ...
            <Card className="overflow-hidden">
              ...
              <CardContent className="p-6 md:p-8">
                {/* Eyebrow — Product Review + category */}
                ...
                {/* Title — editorial display scale */}
                <h1
                  className="mb-5 font-extrabold text-foreground"
                  style={{
                ...
                {/* Content — split at midpoint with related rides inserted */}
                {(() => {
                  const chunks = (post.content || '').split(/\n\n+/);
                  const mid = Math.max(8, Math.floor(chunks.length / 2));
                  const before = chunks.slice(0, mid).join('\n\n');
                  const after = chunks.slice(mid).join('\n\n');
                  ...
                  return (
                    <>
                      <Prose><ReactMarkdown remarkPlugins={[remarkGfm]} components={linkComponents}>{before}</ReactMarkdown></Prose>
                      {after && (
                        <>
                          <RelatedInline posts={relatedPosts as any} heading="Keep Pedaling" />
                          <Prose><ReactMarkdown remarkPlugins={[remarkGfm]} components={linkComponents}>{after}</ReactMarkdown></Prose>
                        </>
                      )}
                    </>
                  );
                ...
                {/* Amazon subscription bounties — free-trial CTA (DB-driven) */}
                <BountyCTA siteId={(site as any).id} />
