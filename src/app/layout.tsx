...
// NOTE: weights deliberately differ from cormorantHeading below — next/font DEDUPES
// identical font configs, which merged --font-cormorant and --font-heading into one
// class and silently overrode every site's chosen heading font with Cormorant.
...
const fraunces = Fraunces({ subsets: ['latin'], variable: '--font-heading', weight: ['400', '500', '600', '700'], display: 'swap' });
const jetbrainsMono = JetBrains_Mono({ subsets: ['latin'], variable: '--font-heading', weight: ['500', '600', '700'], display: 'swap' });
const sora = Sora({ subsets: ['latin'], variable: '--font-heading', weight: ['500', '600', '700', '800'], display: 'swap' });
const syne = Syne({ subsets: ['latin'], variable: '--font-heading', weight: ['600', '700', '800'], display: 'swap' });
const cormorantHeading = Cormorant_Garamond({ subsets: ['latin'], variable: '--font-heading', weight: ['500', '600', '700'], display: 'swap' });
        ...
        {ga4Id && (
          <>
            ...
            <Script id="ga4-init" strategy="afterInteractive">
              {`window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('set',{site_slug:${JSON.stringify(site.slug)},niche:${JSON.stringify(site.niche || '')}});gtag('config','${ga4Id}');document.addEventListener('click',function(e){var t=e.target;if(!t||!t.closest)return;var a=t.closest('a[href*="/go/"]');if(!a||!window.gtag)return;var h=a.getAttribute('href')||'';var i=h.indexOf('/go/');if(i<0)return;var rest=h.slice(i+4);var s=rest.split('?')[0].split('#')[0];if(!s)return;var b=(a.dataset&&a.dataset.brand)||s.split('-').slice(0,2).join(' ');var p=(a.dataset&&a.dataset.placement)||'unknown';window.gtag('event','affiliate_click',{event_category:'conversion',affiliate_brand:b,brand:b,placement:p,slug:s});},true);`}
            </Script>
          </>
        )}
