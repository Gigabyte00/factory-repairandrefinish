...
/**
 * RFC 8058 one-click unsubscribe. Gmail/Yahoo POST here (body: List-Unsubscribe=One-Click)
 * when the List-Unsubscribe-Post header is present. Same effect as GET; returns JSON.
 */
export async function POST(request: Request) {
  const forwarded = (request as any).headers?.get?.('x-forwarded-for');
  const ip = forwarded?.split(',')[0].trim() || 'unknown';
  const { allowed } = rateLimit(`unsub-post:${ip}`, 30, 15 * 60 * 1000);
  if (!allowed) {
    return NextResponse.json({ error: 'Too many requests' }, { status: 429 });
  }
