...
export async function POST(request: NextRequest) {
  try {
    ...
    if (path) {
      revalidatePath(path);
      // GEO: notify IndexNow (Bing/Copilot + the index behind ChatGPT-search
      // citations) that this URL changed. Fire-and-forget; never blocks/fails
      // the revalidation response.
      const indexNow = await pingIndexNow([path]);
