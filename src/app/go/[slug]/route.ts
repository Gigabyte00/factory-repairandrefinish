...
export async function GET(
  ...
  try {
    ...
    if (error || !offer) {
      // No offer row for this slug — usually a content link to an offer that was never
      // created. Salvage the click the same way inactive offers do:-niche sites
      // redirect to a TAGGED Amazon search derived from the slug, instead of a dead 404.
      // Service-niche sites (no Amazon tag) keep the original 404.
      const amazonTag = await getSiteAmazonTag(site.id);
      if (amazonTag) {
        const searchTerm =
          slug.replace(/-[a-z0-9]{6}$/i, '').replace(/-/g, ' ').split(/\s+/).slice(0, 6).join(' ').trim() ||
          slug.replace(/-/g, ' ');
