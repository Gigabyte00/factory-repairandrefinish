...
/**
 * RFC 8058 / Gmail-Yahoo 2024 one-click unsubscribe headers, per recipient.
 * Advertises the same /api/unsubscribe endpoint the footer links to (which now
 * handles POST for one-click). Safe no-op if the endpoint is unreachable.
 */
function unsubHeaders(email: string): Record<string, string> {
  const site = getSiteConfig();
  const domain = (site as any).domain || `${(site as any).slug}.vercel.app`;
  const url = buildUnsubscribeUrl(email, (site as any).id, domain);
  return {
...
export function sendLeadMagnetEmail(email: string, magnet: LeadMagnet): void {
  ...
  sendEmail({
    ...
    html: buildLeadMagnetEmailHtml(magnet, downloadUrl, email),
  });
}
