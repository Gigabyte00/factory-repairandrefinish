...
function getSiteInfo() {
  ...
  // CAN-SPAM: a valid physical postal address must appear in commercial mail.
  // Supplied by the owner at email activation (env), so no placeholder is fabricated.
  const postalAddress = process.env.SENDER_POSTAL_ADDRESS?.trim() || undefined;
  return { id, name, domain, niche, primaryColor, logoUrl, postalAddress };
}

/** Footer unsubscribe URL for a given recipient (empty string if no email). */
function unsubscribeUrlFor(email: string): string {
  const { id, domain } = getSiteInfo();
  return email ? buildUnsubscribeUrl(email, id, domain) : '';
}
...
function layout(title: string, body: string, kind: EmailKind = 'drip', unsubscribeUrl?: string): string {
  const { name, domain, postalAddress } = getSiteInfo();
  const unsubLink = unsubscribeUrl
    ? `<a href="${escapeHtml(unsubscribeUrl)}" style="color:#6b7280;text-decoration:underline">Unsubscribe</a> &middot; `
    : '';
  const addressLine = postalAddress
    ? `<p style="margin:8px 0 0;color:#9ca3af;font-size:12px">${escapeHtml(postalAddress)}</p>`
    : '';
  return `<!DOCTYPE html>
<html lang="en">
...
${unsubLink}<a href="https://${domain}/privacy" style="color:#6b7280;text-decoration:underline">Privacy</a> &middot;
...
`, 'welcome', unsubscribeUrlFor(email));
}
...
`, 'transactional', email ? unsubscribeUrlFor(email) : undefined);
}
...
export function buildSequenceStepHtml(
  ...
  return layout(subject, body, 'drip', unsubscribeUrlFor(vars.email));
}
